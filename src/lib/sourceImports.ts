...
/**
 * @deprecated
 * ASM 콘솔의 ON/OFF 는 일시 상태(전원/네트워크)일 뿐, 타입이 아니다.
 * 콘솔에 잡힌(특히 최근 1달 내 사용이력 있는) 기기는 정의상 모두 '온라인' 타입.
 * OFF(그 시점 꺼짐)도 '온라인' 타입이며, 'M)알약 온라인구분'의 분류값으로
 * '오프라인'은 존재하지 않는다(금지). 네트워크 미연결 독립기기는 '단독형'(구 폐쇄망)이고
 * ASM 콘솔로는 판별 불가(현장 확정). 따라서 접속상태→온라인구분 직접 매핑은 하지 않는다.
 *
 * 함수는 외부 호환을 위해 남겨두며 새 코드에서 호출하지 않는다.
 */
export const normalizeOnlineStatus = (raw: string): string => {
    if (!raw) return '';
    const s = String(raw).trim().toUpperCase();
    // ON/OFF 모두 콘솔 등재 = 온라인 타입(OFF 는 일시 전원오프). '오프라인' 분류값은 만들지 않음.
    if (s === 'ON' || s === 'OFF') return '온라인';
    return raw;
};
